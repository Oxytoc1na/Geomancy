...
public class PedestalBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    private static VoxelShape SHAPE = PedestalBlock.createCuboidShape(0,0,0,16,16,16);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if(state.getBlock() != newState.getBlock()){
            ...
            if(blockEntity instanceof PedestalBlockEntity pedestal){
                ItemScatterer.spawn(world, pos, pedestal.getItems());
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.PEDESTAL_BLOCK_ENTITY,
