...
import org.oxytocina.geomancy.blocks.fluids.ModFluid;
...
public class EntityApplyFluidsMixinNoSinytra {
    ...
    public boolean geomancy$updateMovementInFluid(FluidState instance, TagKey<Fluid> tag, Operation<Boolean> original) {
        if (original.call(instance, tag)) return true;
        return
                tag == FluidTags.WATER ? original.call(instance, ModFluidTags.SWIMMABLE_FLUID)
                : false;
    }

    // Used to cache the state being submerged in water, which is used for initiating swimming.
    // Expanded by including Geomancy's swimmable fluids in the check.
    ...
    public boolean geomancy$updateSubmergedInWaterState(Entity instance, TagKey<Fluid> tag, Operation<Boolean> original) {
        if (original.call(instance, tag)) return true;
        return
                tag == FluidTags.WATER ? original.call(instance, ModFluidTags.SWIMMABLE_FLUID)
                : tag == FluidTags.LAVA ? original.call(instance,ModFluidTags.VISCOUS_FLUID)
                : false;
    }
    ...
    private ParticleEffect geomancy$modifySwimmingStartParticles(ParticleEffect particleEffect) {
        ...
        if (fluid instanceof ModFluid modFluid) {
            return modFluid.getSplashParticle();
        }
    ...
    public void geomancy$updateMovementInFluid(TagKey<Fluid> tag, double speed, CallbackInfoReturnable<Boolean> info, Box box, int i, int j, int k, int l, int m, int n, double d, boolean bl, boolean bl2, Vec3d vec3d, int o, BlockPos.Mutable mutable, int p, int q, int r, FluidState fluidState) {
        ((TouchingFluidAware) this).geomancy$setTouchingExtinguishingFluid(fluidState.isIn(ModFluidTags.EXTINGUISHING_FLUID));
    }
}