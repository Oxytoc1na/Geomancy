...
public class StellgeEngineerModel extends ModEntityModel<StellgeEngineerEntity> implements ModelWithHead {
    ...
    public void setAngles(StellgeEngineerEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        entity.idleAnimationState.startIfNotRunning(0);
        this.updateAnimation(entity.idleAnimationState, StellgeEngineerAnimations.IDLE, ageInTicks);
    }
    private void setHeadAngle(float yaw, float pitch) {
        this.sphere.pitch = pitch * (float) (Math.PI / 180.0);
        this.sphere.yaw = yaw * (float) (Math.PI / 180.0);
    }
